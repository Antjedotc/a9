public class Quadtree {
    ...
    */
    ...
    public double MeanSquaredError(Node n) {
        // root is the mean color of the entire image
        Color Ci = n.color; // Color of node
        Color[][] canvas = img.getCanvas();
        ...
        for (int i = n.xCoord; i < (n.xCoord + n.width); i++){
            for (int j = n.yCoord; j < (n.yCoord + n.height); j++){
                rDiff = canvas[i][j].r() - Ci.r();
                gDiff = canvas[i][j].g() - Ci.g();
                bDiff = canvas[i][j].b() - Ci.b();
        ...
        //     System.out.println("*** countsquared: " + (count * count));
        //     System.exit(0);
        //     return 0;
        // }
        return (count == 0) ? 0 : mse / (count * count);
        // return mse / (count * count);
    }
